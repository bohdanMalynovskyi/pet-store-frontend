    ...
    def wrapper(request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            try:
                ...
                cart = Cart.objects.select_related('unregistered_user', 'unregistered_user__hash_code').values(
                    'id').get(
                    unregistered_user__hash_code__key=hash_code)
                set_interact.delay(cart_id=cart['id'])  # set "last interact" field to now
                return view_func(request, cart['id'], *args, **kwargs)
    ...
    def wrapper(request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            try:
                ...
                featured = FeaturedProducts.objects.select_related('unregistered_user',
                                                                   'unregistered_user__hash_code').values('id').get(
                    unregistered_user__hash_code__key=hash_code)
                set_interact.delay(featured_id=featured['id'])  # set "last interact" field to now
                return view_func(request, featured['id'], *args, **kwargs)
            except (AttributeError, FeaturedProducts.DoesNotExist):
                return Response({'error': 'Token is invalid or not provided'}, status=status.HTTP_400_BAD_REQUEST)
...
def get_cart(cart_id):
    ...
    cart = Cart.objects.select_related('unregistered_user', 'unregistered_user__hash_code', 'user').prefetch_related(
...
def get_featured(featured_id):
    ...
    featured = FeaturedProducts.objects.select_related('unregistered_user', 'unregistered_user__hash_code',
